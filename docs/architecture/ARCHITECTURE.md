...
平台页面通过 `GameRegistry` 获取 metadata，通过 `GameModuleRegistry` 将可用游戏懒加载到 `/play/:gameId`，游戏实现与视觉资源不进入平台首屏 bundle。麻将 Core 不依赖 React、DOM、window、storage 或 CSS；UI 只派发 Action 并读取合法动作和座位可见状态。麻将 UI 通过本地 adapter 将 34 种业务 Tile Type 映射为第三方 SVG 牌面，Core 不知道资源包的存在。`PlatformStorage` 仍是 localStorage 唯一访问边界，牌局本身不持久化。当前没有全局复杂 store、API client 或服务端。
