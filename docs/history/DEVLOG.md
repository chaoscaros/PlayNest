...
## 2026-09-01 - 单人对战基础 AI

- 根据用户试玩反馈取消四人共享设备换手，玩家固定坐东家，南、西、北改为三名自动操作的基础 AI。
- 新增独立纯策略层：acting seat 解析、能胡必胡、碰/吃/过响应，以及基于对子、相邻牌和搭子价值的确定性弃牌。
- AI 只产生 `MahjongAction` 并继续经过 `getLegalActions` 与 `applyMahjongAction`，不直接修改 State，也不以对手隐藏手牌制定策略。
- UI 开局直接显示玩家 14 张手牌；电脑操作时持续显示玩家手牌和思考状态，不再出现换手遮罩。
- 更新目录 metadata 为 `single`，详情页与规则说明同步为单人对战三名电脑。
- 新增 AI 策略和单人 UI 测试。运行时复用用户已有 `localhost:9999` 标签页，确认三名电脑连续摸打、副露并把操作权交回玩家；桌面与 375×812 手机端保持单屏。本轮未自行启动开发服务。
