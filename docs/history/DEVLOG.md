...
- 修复将规则座位名称直接绑定屏幕方位导致“东家玩家显示在右侧”的问题。
- 新增独立视角映射：东家玩家在下、南家在右、西家在上、北家在左；Core 座次与响应顺序不变。
- UI 测试加入四个座位的方位 class 断言。
- 复用用户已有 `localhost:9999` 标签页确认桌面与 375×812 手机端东家均位于下方，页面保持单屏；本轮未自行启动开发服务。

## 2026-09-01 - 基础麻将加入杠系统

### 目标

在现有单人 AI 基础麻将上补齐暗杠、明杠、加杠与杠后补牌，不引入番型、算分、抢杠胡或联网。

### 三种杠与补牌

- 暗杠从玩家手牌移动四张同类实体，牌桌只显示四张牌背。
- 明杠取得当前弃牌并移动玩家手中三张同类实体，弃牌不再留在弃牌区。
- 加杠使用玩家手中第四张升级已有碰，复用原副露三张实体。
- 普通摸牌取牌墙头部，杠后补牌取尾部；无补牌可用时直接流局。

### Reaction 与 AI

响应顺序更新为 `胡 > 明杠 > 碰 > 吃`，同一玩家可在明杠、碰、过之间选择。基础 AI 保持胡优先，有合法暗杠、加杠或明杠时通过 Core Action 执行，连续杠由逐次状态更新驱动。

### UI

操作区按合法动作显示杠按钮和具体牌型，多个可杠组合分别可选；暗杠使用现有牌体风格的统一牌背，明杠和加杠显示四张真实 SVG 牌面。手机操作按钮允许紧凑换行。

### 测试

扩展 Core、Solver、AI 与 React UI 测试，覆盖三种杠、墙头/墙尾摸牌、连续杠、杠后胡、无补牌流局、响应优先级、暗杠信息隐藏以及 136 张唯一 TileInstance 守恒。

### 已知限制

无抢杠胡、无杠分、无番、无算分、无花牌、无特殊胡型、无联机；AI 仍是确定性的规则验证级基础策略。

### 下一步

等待用户试玩加入杠后的基础麻将，再由用户选择基础番型与简单计分、继续完善麻将规则、在线房间基础或第二款游戏。
